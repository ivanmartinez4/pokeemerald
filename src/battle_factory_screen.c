...
static void Select_CreateMonSprite(void)
{
    ...
    sFactorySelectScreen->monPics[1].monSpriteId = CreateMonPicSprite(species, otId, personality, MON_PIC_NO_AFFINE_FRONT, 88, 32, 15, TAG_NONE);
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < SELECTABLE_MONS_COUNT; j++)
        {
            if (sFactorySelectScreen->mons[j].selectedId == i + 1)
            {
                ...
                sFactorySelectScreen->monPics[i].monSpriteId = CreateMonPicSprite(species, otId, personality, MON_PIC_NO_AFFINE_FRONT, (i * 72) + 16, 32, i + 13, TAG_NONE);
...
#ifdef BUGFIX
    sFactorySwapScreen->monPic.monSpriteId = CreateMonPicSprite(species, otId, personality, MON_PIC_NO_AFFINE_FRONT, 88, 32, 15, TAG_NONE);
#else
    sFactorySwapScreen->monPic.monSpriteId = CreateMonPicSprite(species, personality, otId, MON_PIC_NO_AFFINE_FRONT, 88, 32, 15, TAG_NONE);
#endif
